  ...
  return (
    <button
      ...
      className={cn(
        ...
        selected ? 'border-pink-600 bg-pink-50' : 'border-border bg-card hover:border-pink-300'
      )}
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="text-xs text-pink-700">{formatCOP(upsell.price)}</div>
      </div>
      <span className={cn('grid h-6 w-6 shrink-0 place-items-center rounded-full border-2 transition-all',
        selected ? 'border-pink-600 bg-pink-600 text-white' : 'border-border text-muted-foreground')}>
